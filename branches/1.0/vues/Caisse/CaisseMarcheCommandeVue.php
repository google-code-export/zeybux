...
if( isset($_SESSION[DROIT_ID]) && ( isset($_SESSION[MOD_CAISSE]) || isset($_SESSION[DROIT_SUPER_ZEYBU]) ) ) {
	...
	if(isset($_POST['id_commande'])) {		
		if(isset($_POST['id_adherent'])) {	
			...
			$lLogger->log("Affichage de la vue AchatCommande par le compte de l'Adhérent : " . $_SESSION[DROIT_ID],PEAR_LOG_INFO);	// Maj des logs
		} else {
			echo $lControleur->getListeAdherentCommande($_POST['id_commande'])->exportToJson();
			$lLogger->log("Affichage de la vue MarcheCommande par le compte de l'Adhérent : " . $_SESSION[DROIT_ID],PEAR_LOG_INFO);	// Maj des logs
		}	
		...
		if($lResponse->getValid()) {
			$lLogger->log("Enregistrement d'un achat par l'adherent : " . $_SESSION[DROIT_ID],PEAR_LOG_INFO);	// Maj des logs
		} else {
			$lLogger->log("Echec de l'enregistrement d'un achat par l'adherent : " . $_SESSION[DROIT_ID],PEAR_LOG_INFO);	// Maj des logs
		}
	} else {
		$lLogger->log("Demande d'affichage de la vue MarcheCommande sans id de commande par le compte de l'Adhérent : " . $_SESSION[DROIT_ID],PEAR_LOG_INFO);	// Maj des logs
	}
} else {
