...
if( isset($_SESSION[DROIT_ID]) && ( isset($_SESSION[MOD_GESTION_PRODUCTEUR]) || isset($_SESSION[DROIT_SUPER_ZEYBU]) ) ) {	
	if(isset( $_POST['pParam'] )) {
		...
		if(isset($lParam['id_producteur']) && !empty($lParam['id_producteur'])) {
			...
			$lLogger->log("Affichage de la vue modification du producteur par : " . $_SESSION[ID_CONNEXION],PEAR_LOG_INFO);	// Maj des logs
			...
			if($lResponse->getValid()) {
				$lLogger->log("Modification du producteur par : " . $_SESSION[ID_CONNEXION],PEAR_LOG_INFO);	// Maj des logs
			} else {
				$lLogger->log("Echec de la modification du producteur par : " . $_SESSION[ID_CONNEXION],PEAR_LOG_INFO);	// Maj des logs
			}
			
		} else {
			$lLogger->log("Demande d'accés sans paramètre à la modification des producteurs par : " . $_SESSION[ID_CONNEXION],PEAR_LOG_INFO);	// Maj des logs
