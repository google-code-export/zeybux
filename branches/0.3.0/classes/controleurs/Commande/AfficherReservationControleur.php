...
include_once(CHEMIN_CLASSES_RESPONSE . "AfficherReservationResponse.php" );
...
include_once(CHEMIN_CLASSES_VALIDATEUR . "SupprimerReservationValid.php");
...
class AfficherReservationControleur
{
	...
	*/
	public function modifierReservation($pParam) {
		...
		// TODO il faut un test pour vérifier si la commande n'est pas cloturée
	...
	*/
	public function supprimerReservation($pParam) {
		...
		$lVr = SupprimerReservationValid::validAjout($pParam);
		if($lVr->getValid()) {
			$lIdCommande = $pParam['id_commande'];
