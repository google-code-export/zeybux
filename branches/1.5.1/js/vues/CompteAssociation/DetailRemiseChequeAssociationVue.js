;function DetailRemiseChequeAssociationVue(pParam) {
	this.mEtat = null;
	this.mIdRemise = null;
	
	this.construct = function(pParam) {
		$.history( {'vue':function() {DetailRemiseChequeAssociationVue(pParam);}} );
		var that = this;		
		pParam.fonction = 'detailRemise';
		$.post(	"./index.php?m=CompteAssociation&v=RemiseCheque", "pParam=" + $.toJSON(pParam),
			function(lResponse) {
	...
	this.afficher = function(pResponse) {
		...
		$('#contenu').replaceWith(that.affect($(lCompteAssociationTemplate.detailRemiseCheque.template(pResponse.remiseCheque))));
	};
	...
	this.affectExport = function(pData) {
		pData.find('#btn-export').click(function() {
			$.download("./index.php?m=CompteAssociation&v=RemiseCheque", {fonction:'export',id:$(this).data('id')});
		});
		return pData;
	};
	...
	this.affectRetour = function(pData) {
		...
		pData.find("#lien-retour").click(function() {
			ListeRemiseChequeAssociationVue({etat:that.mEtat});
		});
		return pData;
	};
	...
	this.dialogSupprimerOperationRemiseCheque = function(pId) {
		var that = this;
		var lCompteAssociationTemplate = new CompteAssociationTemplate();
		$(lCompteAssociationTemplate.dialogSupprimerOperationRemiseCheque).dialog({
			autoOpen: true,
			modal: true,
			draggable: false,
			resizable: false,
	...
	this.dialogValiderRemiseCheque = function(pId) {
		var that = this;
		var lCompteAssociationTemplate = new CompteAssociationTemplate();
		$(lCompteAssociationTemplate.dialogValiderRemiseCheque).dialog({
			autoOpen: true,
			modal: true,
			draggable: false,
			resizable: false,
	...
	this.dialogSupprimerRemiseCheque = function(pId) {
		var lCompteAssociationTemplate = new CompteAssociationTemplate();
		$(lCompteAssociationTemplate.dialogSupprimerRemiseCheque).dialog({
			autoOpen: true,
			modal: true,
			draggable: false,
			resizable: false,
							...
							function(lResponse) {
								...
								if(lResponse) {
									if(lResponse.valid) {
										...
										lVr.log.erreurs.push(erreur);						
										lDialog.dialog("close");	
										ListeRemiseChequeAssociationVue({vr:lVr});									
									} else {
										Infobulle.generer(lResponse,'');
									}
								}
