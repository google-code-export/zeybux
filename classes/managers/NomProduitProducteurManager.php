...
define("TABLE_NOMPRODUITPRODUCTEUR", MYSQL_DB_PREFIXE . "nprdt_nom_produit_producteur");
/**
...
{
	const TABLE_NOMPRODUITPRODUCTEUR = TABLE_NOMPRODUITPRODUCTEUR;
	...
	*/
	...
	/**
	* @name selectIdProducteurByIdProduit($pId)
	* @param integer
	* @return array(integer)
	* @desc Récupère la liste des id producteurs pour l'Id Produit et le retourne sous forme de collection d'entier
	*/
	/*public static function selectIdProducteurByIdProduit($pId) {
		// Initialisation du Logger
		$lLogger = &Log::singleton('file', CHEMIN_FICHIER_LOGS);
		$lLogger->setMask(Log::MAX(LOG_LEVEL));
		...
		$lRequete =
			...
			FROM " . NomProduitProducteurManager::TABLE_NOMPRODUITPRODUCTEUR . " 
			WHERE " . NomProduitProducteurManager::CHAMP_NOMPRODUITPRODUCTEUR_ID . " = '" . StringUtils::securiser($pId) . "'
			AND " . NomProduitProducteurManager::CHAMP_NOMPRODUITPRODUCTEUR_ETAT . " = '0';";
		...
		if( mysql_num_rows($lSql) > 0 ) {
			while ($lLigne = mysql_fetch_assoc($lSql)) {
				array_push($lListeNomProduitProducteur,NomProduitProducteurManager::CHAMP_NOMPRODUITPRODUCTEUR_ID_PRODUCTEUR);
			}
		}
	...
	* @param array(NomProduitProducteurVO)
	* @desc Insère une nouvelle ligne dans la table, à partir des informations de la NomProduitProducteurVO en paramètre (l'id sera automatiquement calculé par la BDD)
	*/
	public static function insertByArray($pVo) {
		...
		if(is_array($pVo)) {
			...
			VALUES ";
			$lTaille = count($pVo);
			foreach($pVo as $lNomProduitProducteur) {
				$lTaille--;
				if($lTaille > 0) {
					$lRequete .= "(NULL
					,'" . StringUtils::securiser( $lNomProduitProducteur->getIdNomProduit() ) . "'
					,'" . StringUtils::securiser( $lNomProduitProducteur->getIdProducteur() ) . "'
					,'" . StringUtils::securiser( $lNomProduitProducteur->getEtat() ) . "'),";
				} else {
					$lRequete .= "(NULL
					,'" . StringUtils::securiser( $lNomProduitProducteur->getIdNomProduit() ) . "'
					,'" . StringUtils::securiser( $lNomProduitProducteur->getIdProducteur() ) . "'
	...
	*/
	public static function deleteByIdNomProduit($pId) {
		...
		$lRequete = 
			"UPDATE " . NomProduitProducteurManager::TABLE_NOMPRODUITPRODUCTEUR . "
			 SET " . NomProduitProducteurManager::CHAMP_NOMPRODUITPRODUCTEUR_ETAT . " = '1'
	...
	public static function deleteByArray($pIds) {
		...
		if(is_array($pIds)) {
			$lRequete = 
				"UPDATE " . NomProduitProducteurManager::TABLE_NOMPRODUITPRODUCTEUR . "
				 SET " . NomProduitProducteurManager::CHAMP_NOMPRODUITPRODUCTEUR_ETAT . " = '1'
				 WHERE " . NomProduitProducteurManager::CHAMP_NOMPRODUITPRODUCTEUR_ID . " in (";
