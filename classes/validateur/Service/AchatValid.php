...
class AchatValid
{
	...
	*/
	...
	public function produits($pProduits) {		
		if(is_array($pProduits) && count($pProduits) > 0) {
			$lValid = true;
			$lProduitAchatValid = new ProduitAchatValid();
	...
	public function input($pAchat) {
		if($this->estAchat($pAchat)) {
			...
			return (
						(
							( !is_null($lOperationAchat->getIdCompte()) 
									&& (	$lOperationValid->insert($pAchat->getOperationAchat()) 
										|| $lOperationValid->update($pAchat->getOperationAchat()) 
										)
							) 
							||
							( !is_null($lOperationAchatSolidaire->getIdCompte()) 
									&& (	$lOperationValid->insert($pAchat->getOperationAchatSolidaire()) 
					...
					|| 
					( !is_null($lRechargement->getIdCompte()) 
							&& (	$lOperationValid->insert($pAchat->getRechargement())
								|| $lOperationValid->update($pAchat->getRechargement()) )
					);
		} else {
			return false;
		}
	...
	*/
	public function select($pIdAchat) {	
		...
		return $lIdValid->estId($pIdAchat) && !empty($pIdAchat);
	}
	...
	*/
	public function selectAll($pIdAchat) {
		...
		$lIdValid = new \IdValid();
		return !empty($pIdAchat)
			&& $lIdValid->estId($pIdAchat->getIdCompte()) && !empty($lIdCompte)
			&& $lIdValid->estId($pIdAchat->getIdCommande()) && !empty($lIdMarche);
	}
}
?>