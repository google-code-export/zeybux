...
include_once(CHEMIN_CLASSES_VR . MOD_COMPTE_ASSOCIATION . "/OperationDetailVR.php" );
...
class OperationDetailValid
{
	...
	*/
	public static function validAjout($pData, $pParam = array("negatif" => false, "reel" => false)) {
		...
		if(!isset($pData['libelle'])) {
			$lVr->setValid(false);
			$lVr->getLibelle()->setValid(false);
			$lErreur = new VRerreur();
			$lErreur->setCode(MessagesErreurs::ERR_201_CODE);
			$lErreur->setMessage(MessagesErreurs::ERR_201_MSG);
		...
		if($lVr->getValid()) {
			...
			if(!TestFonction::checkLength($pData['libelle'],0,100)) {
				$lVr->setValid(false);
				$lVr->getLibelle()->setValid(false);
				$lErreur = new VRerreur();
				$lErreur->setCode(MessagesErreurs::ERR_101_CODE);
			...
			if(!$pParam["reel"] && empty($pData['montant'])) {
				$lVr->setValid(false);
				$lVr->getMontant()->setValid(false);
				$lErreur = new VRerreur();
				$lErreur->setCode(MessagesErreurs::ERR_201_CODE);
			...
			if(!$pParam["reel"] && $pParam["negatif"] && $pData['montant'] > 0) {
				$lVr->setValid(false);
				$lVr->getMontant()->setValid(false);
				$lErreur = new VRerreur();
